                                   ...
                                   intptr_t temp_var)
{
    ...
    {
        if (HASFLAG(setting, F_CHOICE_CFGVALS))
        {
            /* display strings come from cfg_vals */
...
/* Trimpod: do_menu uses this for the iPod inline LEFT/RIGHT value cycling. */
    ...
    while (!done)
    {
        ...
        if (!cb_on_select_only && function)
        {
            if (!cb_on_changed || (*variable != oldvalue))
            {
                ...
                /* the callback may change theme colours (background preview);
                 * redraw so the list isn't a colour behind the screen */
                gui_synclist_draw(&lists);
            }
        }
