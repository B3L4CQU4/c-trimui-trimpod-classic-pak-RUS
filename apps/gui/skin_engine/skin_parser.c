                           ...
                           struct wps_data *wps_data)
{
    ...
    if(element->params_count > 2)
        ...
        glyphs = TRIMPOD_GLYPHS_TO_CACHE;
    if (id < 2)
    {
...
static void skin_data_reset(struct wps_data *wps_data)
{
    ...
    wps_data->spectrum_enabled = false;
...
static int load_skin_bmp(struct wps_data *wps_data, struct gui_img *img, char* bmpdir)
{
    ...
    int bmpformat;
    ssize_t buf_reqd;
        bmpformat = img->dither ? FORMAT_ANY|FORMAT_DITHER :
                                  FORMAT_ANY;
                    ...
                    const char *buf, bool isfile, struct skin_stats *stats)
{
    ...
    size_t buffersize = 0;
    wps_buffer = (char *)scratch_buffer_get(&buffersize);
