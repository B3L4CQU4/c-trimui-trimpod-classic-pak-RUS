                             ...
                             struct skin_element *element)
{
    ...
    switch (token->type)
    {
        ...
        case SKIN_TOKEN_UIVIEWPORT_ENABLE:
            ...
            break;
        case SKIN_TOKEN_SPECTRUM:
            data->spectrum_enabled = true;
            if (do_refresh)
                draw_spectrum(gwps, info->line_number, &skin_vp->vp);
            break;
                                          ...
                                          struct skin_draw_info *info)
{
    ...
    else if (branch->type == LINE && branch->children_count)
    {
        ...
        while (child)
        {
            ...
            else if (token->type == SKIN_TOKEN_SPECTRUM)
            {
                data->spectrum_enabled = false;
            }
...
void skin_render(struct gui_wps *gwps, unsigned refresh_mode)
{
    ...
    /* The shared status bar composites ON TOP of the WPS.  By the time it draws,
     * the WPS has already cleared the framebuffer and drawn its body, so the SBS
     * must OVERLAY -- draw its content on top and clear NOTHING, neither the full
     * screen nor its own viewports -- otherwise it wipes WPS content it overlaps
     * (the metadata, the time counters).  Outside the WPS (menus, USB, etc.) the
     * SBS owns the screen and clears normally. */
    bool sbs_over_wps = (get_current_activity() == ACTIVITY_WPS) &&
                        (gwps == skin_get_gwps(CUSTOM_STATUSBAR,
                                               display->screen_type));
    ...
    if ((refresh_mode&SKIN_REFRESH_ALL) == SKIN_REFRESH_ALL)
    {
        ...
        if (!sbs_over_wps &&
            (first_vp->flags & VP_FLAG_VP_SET_CLEAN) == VP_FLAG_VP_DIRTY &&
         ...
         viewport = SKINOFFSETTOPTR(skin_buffer, viewport->next))
    {
        ...
        /* sbs_over_wps: overlay only -- don't clear this viewport's region, or an
         * SBS viewport that overlaps WPS content (e.g. the empty "Now Playing"
         * strip sitting over the time counters) would erase it. */
        if (!sbs_over_wps && (vp_refresh_mode&SKIN_REFRESH_ALL) == SKIN_REFRESH_ALL)
        {
            display->clear_viewport();
        }
        /* render */
