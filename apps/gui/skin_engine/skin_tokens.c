...
#include "tdspeed.h"
#include "viewport.h"
#include "trimpod_ui.h"   /* Trimpod: header-legend string for the "Tl" token */
                          ...
                          char *filename, char *buf, int buf_size, int limit, int *intval)
{
    ...
    if (id3)
    {
        ...
        switch (token->type)
        {
            ...
            {
                /* Count the remaining time down from the *whole-second* elapsed
                 * value so it ticks in lock-step with the count-up (%pc).
                 * Flooring the raw (length - elapsed) instead picks a different
                 * sub-second phase, so for tracks whose length isn't an exact
                 * number of seconds the two timers update up to ~1s apart. */
                unsigned long remaining = length - (elapsed - elapsed % 1000);
                if (intval && limit == TOKEN_VALUE_ONLY)
                    *intval = remaining/1000;
                format_time(buf, buf_size, remaining);
                return buf;
            }
    ...
    switch (token->type)
    {
        ...
        case SKIN_TOKEN_VOLUME:
            ...
            if (intval)
            {
                ...
                /* Trimpod: the volume bar spans [sound_min .. Maximum Volume
                 * Limit] and fills COMPLETELY at the limit (matching the dB
                 * book-end labels %vn / %vx), instead of always scaling to the
                 * hardware 0 dB top. */
                int maxvol = global_settings.volume_limit;
                if (maxvol <= minvol)
                ...
                else
                {
                    ...
                    numeric_ret = 1 + (limit - 1) * (v - minvol) / (maxvol - minvol);
                }
            }
            numeric_buf = buf;
            goto gtv_ret_numeric_tag_info;

        case SKIN_TOKEN_VOLUME_MIN:
            /* lowest volume (sound_min); skin_token=true => bare number, the
             * theme appends the "dB" unit */
            format_sound_value_ex(buf, buf_size, SOUND_VOLUME,
                                  sound_min(SOUND_VOLUME), true);
            return buf;

        case SKIN_TOKEN_VOLUME_LIMIT:
        ...
        case SKIN_TOKEN_TRIMPOD_LEGEND:
            /* Trimpod: the active page's header button-legend (or NULL); the SBS
             * overlays it on the status row.  See trimpod_set_header_legend(). */
            return trimpod_get_header_legend();
