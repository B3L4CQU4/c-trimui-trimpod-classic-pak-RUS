                      ...
                      void (*function)(bool))
{
    ...
    struct opt_items names[] = {
        {(unsigned const char *)no_str},
        {(unsigned const char *)yes_str}
    };
    bool result;
...
{
    return set_int_ex(string, unit, variable, function,
...
#define SETTINGFLAGS(n)                  if(flags_rem & n) {              flags_rem &= ~n;              strlcat(buf, "["#n"]", sizeof(buf));}
