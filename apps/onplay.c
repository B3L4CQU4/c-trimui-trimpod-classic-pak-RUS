...
MAKE_ONPLAYMENU( wps_playlist_menu, ID2P(LANG_CURRENT_PLAYLIST),
                 ...
                 &search_playlist_item, &reshuffle_item
               );
...
static void op_playlist_insert_selected(int position, bool queue)
{
    ...
    {
        bool recurse = (TP_RECURSIVE_DIR_INSERT == RECURSE_ON);
        if (TP_RECURSIVE_DIR_INSERT == RECURSE_ASK)
        {
...
static int add_to_playlist(void* arg)
{
    ...
    if (new_playlist && TP_KEEP_CURRENT_ON_REPLACE)
    {
                                 ...
                                 struct gui_synclist *this_list)
{
    ...
    case ACTION_REQUEST_MENUITEM:
        ...
        {
            if (TP_SHOW_QUEUE_OPTIONS != QUEUE_SHOW_IN_SUBMENU)
                return ACTION_EXIT_MENUITEM;
                 ...
                 this_item->function_param->function_w_param == add_to_playlist)
        {
            ...
            {
                if (TP_SHOW_QUEUE_OPTIONS != QUEUE_SHOW_AT_TOPLEVEL &&
                    !in_queue_submenu)
                    return ACTION_EXIT_MENUITEM;
            }
