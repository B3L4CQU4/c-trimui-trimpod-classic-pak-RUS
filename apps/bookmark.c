                                            ...
                                            uint32_t *track_hash)
{
    ...
    *track_hash = 0;
    ...
    if (BM_USEMRB == BOOKMARK_ONE_PER_TRACK)
    {
                         ...
                         bool most_recent)
{
    ...
    if (most_recent && ((BM_USEMRB == BOOKMARK_ONE_PER_PLAYLIST)
                      || (BM_USEMRB == BOOKMARK_ONE_PER_TRACK)))
    {

        if (bookmark_get_playlist_and_track_hash(bookmark, &pl_hash, &pl_track_hash))
        {
            comp_playlist = true;
            comp_track = (BM_USEMRB == BOOKMARK_ONE_PER_TRACK);
        }
    }
                           ...
                           char** selected_bookmark)
{
    ...
    *selected_bookmark = NULL;
    ...
    bookmarks = scratch_buffer_get(&size);
