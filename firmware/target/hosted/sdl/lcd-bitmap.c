...
/* Trimpod: when set, on-screen pushes are dropped so a page can be rendered to
 * the framebuffer off-screen (used by the slide transition to prep the incoming
 * frame without a flash).  The framebuffer is still written; only the present
 * to the window is skipped. */
static bool lcd_update_suppressed = false;
void lcd_set_update_suppressed(bool suppress)
{
