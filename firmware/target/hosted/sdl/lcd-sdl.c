...
{
    SDL_Rect dest;
/* Fast path for RGB565/RGB888: the framebuffer and lcd_surface are both
 * LCD_DEPTH zero-mask surfaces, so one SDL_BlitSurface is a same-format bulk
 * copy -- vs. the per-pixel SDL_FillRect fallback below (~76k calls/frame). */
#if LCD_DEPTH >= 8 && (LCD_PIXELFORMAT == RGB565 || LCD_PIXELFORMAT == RGB888) && \
                    ...
                    int height, int max_x, int max_y, int ui_x, int ui_y)
{
    ...
    /* Trimpod presents the whole sim_lcd_surface via OpenGL (see window-sdl.c).
     * A partial viewport update must therefore be blitted to its real position
     * (dest); the SDL_Renderer path below relies on SDL_UpdateTexture to place
     * the region instead, so it blits to (0,0). Blitting to NULL here would put
     * every partial update in the top-left corner. */
    SDL_BlitSurface(surface, &src, sim_lcd_surface, &dest);
