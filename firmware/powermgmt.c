...
#if (!(CONFIG_BATTERY_MEASURE & TIME_MEASURE) &&  (defined(CURRENT_NORMAL) || (CONFIG_BATTERY_MEASURE & CURRENT_MEASURE)))
#endif

#if !(CONFIG_BATTERY_MEASURE & PERCENTAGE_MEASURE)
int _battery_level(void) { return -1; }
#endif
static int percent_now = 100; /* Cached; start full until the first real poll
                               * (avoids an empty-bar flash / false low battery) */
static bool battery_polled = false; /* true once a real level has been read */
...
/* Display-only battery level: shows a full bar on load, then eases ("cycles")
 * down to the real value once the first poll lands.  Used by the skin so the
 * status bar never flashes a stale/empty reading at startup. */
int battery_level_display(void)
{
    int real = battery_level();
    if (real < 0 || !battery_polled)
        return real < 0 ? real : 100;   /* no battery info / not polled: full */

    static long sweep_start = 0;
    static bool done = false;
    if (done)
        return real;
    if (sweep_start == 0)
        sweep_start = current_tick;     /* arm on first render after the poll */

    long dt = current_tick - sweep_start;
    const long dur = HZ;                 /* ~1s sweep from 100% to the real level */
    if (dt >= dur || real >= 100) {
        done = true;
        return real;
    }
    return 100 - (int)((long)(100 - real) * dt / dur);
}
...
static void handle_sleep_timer(void)
{
    if (TIME_AFTER(current_tick, sleeptimer_endtick)) {
        ...
        /* NextUI-style deep sleep: pause, blank, suspend-to-RAM; the call blocks
         * until the power button wakes us, then we carry on (timer disarmed). */
        DEBUGF("Sleep timer timeout. Deep sleeping...\n");
        audio_pause();
        set_sleep_timer(0);
        backlight_off();
        trimui_deep_sleep();
...
void handle_auto_poweroff(void)
{
    long timeout = poweroff_timeout*HZ;   /* idle poweroff value is in seconds */
         ...
         audio_stat == (AUDIO_STATUS_PLAY | AUDIO_STATUS_PAUSE)))
    {
        if (TIME_AFTER(tick, last_event_tick + timeout)
            ...
            trimui_safe_poweroff();  /* launcher does the real PMIC poweroff on exit */
#endif
            sys_poweroff();
        }
    }
