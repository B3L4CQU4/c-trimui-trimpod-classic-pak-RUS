...
int battery_level_display(void); /* skin-only: full on load, eases to real level */
void handle_auto_poweroff(void);
void reset_poweroff_timer(void);
void cancel_shutdown(void);
void shutdown_hw(enum shutdown_type sd_type);
#ifdef HAVE_TRIMUI_SAFE_POWEROFF
/* Flag the launcher to run its safe PMIC poweroff (poweroff_next) once we exit. */
void trimui_safe_poweroff(void);
#endif
#ifdef HAVE_TRIMUI_DEEP_SLEEP
/* NextUI-style deep sleep: run the launcher's `suspend` script (echo mem >
 * /sys/power/state). Blocks until the power button wakes the device. */
