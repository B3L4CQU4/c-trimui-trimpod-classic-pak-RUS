...
long            embolden_strength = 0; /* FT 26.6 units; negative = thinner (-e) */
int             oflag       = 0;
char            outfile[1024];
    ...
    while (ac > 0 && av[0][0] == '-') {
        ...
        while( *p)
            ...
            case 'p':       /* set pixel size*/
                ...
                break;
            case 'e':       /* embolden strength in 1/64 px (negative = thinner) */
                if (*p) {
                    embolden_strength = atol(p);
                    while (*p && *p != ' ')
                        p++;
                }
                else {
