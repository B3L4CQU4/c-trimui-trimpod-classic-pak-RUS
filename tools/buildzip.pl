...
sub buildzip {
    ...
    if($fonts) {
        ...
        my $cmd = "$ROOT/tools/convbdf -f $ROOT/assets/fonts/*bdf >/dev/null 2>&1";
        print($cmd."\n") if $verbose;
        system($cmd);
        copy("$ROOT/assets/fonts/COPYING", "COPYING-fonts.txt");
        chdir("../../");
    ...
    # Trimpod has no plugins, so there are no rocks/viewers.config/CATEGORIES to
    # package -- only the database's tag-navigation config.
    ...
    if(-d "$ROOT/assets/wps") {
        my $wps_build_cmd="perl $ROOT/assets/wps/wpsbuild.pl ";
        $wps_build_cmd=$wps_build_cmd."-v " if $verbose;
        $wps_build_cmd=$wps_build_cmd." --tempdir=$temp_dir --rbdir=$rbdir -r $ROOT -m $modelname $ROOT/assets/wps/WPSLIST $target";
